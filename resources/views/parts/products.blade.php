<div class="col-xs-12 col-md-{{ $columns }} product-item">
    <a href="{{ route('product', [$product->category->code ?? $product->code, $product->id]) }}"><img
            src="{{ url("/images/$product->image") }}" class="img-fluid img-center"></a>
    <div class="product">
        <div class="product_top">
            <p class="product-title"><a href="{{ route('product', [$product->category->code ?? $product->code, $product->id]) }}">{{ $product->name }}</a></p>
            <favorite
                :product={{ $product->id }}
